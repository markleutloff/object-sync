...
describe("Object Serializer", () => {
  ...
  it("should transfer initial data", async () => {
    ...
    const destObject = destSync.findOne(Alpha, "main")!;
  ...
  it("should delete unused references", async () => {
    ...
    assert(sourceSync.findOne(Beta) === undefined);
    assert(destSync.findOne(Beta) === undefined);
  });

  it("invoke should reuse references", async () => {
    await sendDataToDest();

    let clientResultPromise = sourceSync.getDispatcher(sourceObject)!.invoke(sourceSyncDestClientToken, "returnAndStoreBeta");
    await sendDataToDest();
    await sendDataToSource();
    const clientResult0 = await clientResultPromise;

    clientResultPromise = sourceSync.getDispatcher(sourceObject)!.invoke(sourceSyncDestClientToken, "returnAndStoreBeta");
    await sendDataToDest();
  ...
  it("should invoke sync methods", async () => {
    ...
    const input = "hello";
    const clientResultPromise = sourceSync.getDispatcher(sourceObject)!.invoke(sourceSyncDestClientToken, "doSomething", input);
  ...
  it("should invoke async methods", async () => {
    ...
    const input = "hello";
    const clientResultPromise = sourceSync.getDispatcher(sourceObject)!.invoke(sourceSyncDestClientToken, "doSomethingAsync", input);
  ...
  it("invoke should support references", async () => {
    ...
    input.value = "test";
    const clientResultPromise = sourceSync.getDispatcher(sourceObject)!.invoke(sourceSyncDestClientToken, "returnBeta", input, " modified");
  ...
  it("should support custom constructor arguments by property name", async () => {
    ...
    const destGamma = destSync.findOne(Gamma0, "gamma")!;
  ...
  it("should support custom constructor arguments by property names from function", async () => {
    ...
    const destGamma = destSync.findOne(Gamma1, "gamma")!;
    assert(destGamma.value === "custom value");
  });
  ...
  it("should support custom constructor arguments by values from function", async () => {
    ...
    const destGamma = destSync.findOne(Gamma2, "gamma")!;
    assert(destGamma.value === "custom value");
  });
});
